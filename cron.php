...
foreach ($data as $item) {
    ...
    if ($check === false) {
        ...
        //正常的
        //$db->insert('system_domain_api_log', ['name' => $item['name'], 'status' => 1, 'descr' => '域名正常', 'add_time' => date('Y-m-d H:i:s')]);
    }
}
...
function domainCheck($apiToken, $reqUrl)
{
    ...
    file_put_contents(__DIR__ . '/log.log', $reqUrl . '   ' . $responseBody . '   ' . date('Y-m-d H:i:s') . PHP_EOL, FILE_APPEND);
  	//$db->insert('system_domain_api_log', ['name' => $item['name'], 'descr' => $responseBody, 'add_time' => date('Y-m-d H:i:s')]);
